...
if (code!==''){
    try{
        const {data}=await axios.request({method:"POST",url:"https://ecommerce.routemisr.com/api/v1/auth/verifyResetCode",data:{resetCode:code}})
  ...
  async function resetPass(e){
    e.preventDefault()
    if(!newPassword.match(passRole)){return setError('your password should be more thane 8 char and has number, small and large char')}
    else if(inputValue !==''&& newPassword.match(passRole)){
        let waite= toast.loading("Waiting...")
           waite
    try {const {data}=await axios.request({method:"PUT",url:"https://ecommerce.routemisr.com/api/v1/auth/resetPassword",data:{email:inputValue,newPassword}})
   toast.dismiss(waite)
  ...
  return (
    <>
      <div className=" w-1/2 mx-auto flex flex-wrap justify-center">
        ...
        {reset?<div className="ml-1">
            <form>
            <label htmlFor="resrtEmail" className="w-full text-sm  font-bold">
              Enter your E-mail:
            </label>
            <input
              id="resetEmail"
